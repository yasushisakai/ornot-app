<script>
	...
	const my_votes = derived(topic_data, ($topic_data) => {
	...
	const is_participant = derived(topic_data, ($topic_data) => {
		if ($topic_data.setting) {
	...
	const plan_inputs = derived(
		[plans, topic_data, my_votes],
		([$plans, $topic_data, $my_votes]) => {
			if ($topic_data.setting) {
				return $topic_data.setting.plans
					...
					.reduce((fold, plan) => {
						console.log(plan);
						let ph = $my_votes[plan[0]] ? $my_votes[plan[0]] : 0.0;
						return `${fold}<li>${ph} - ${plan[1].data}</li>`;
					}, "");
			} else {
				return "";
			}
	...
	async function add_plan() {
		...
		let simple_plan = { type: "simple", data: option };

		let result = await fetch(`${api_prefix}/topic/${topic_id}/new_plan`, {
			method: "POST",
			body: JSON.stringify(simple_plan),
			headers: { "Content-Type": "application/json" },
		}).then((x) => x.json());

		// addit to the local object 'plans'
		if (result.length == 2) {
			let obj = {};
			obj[result[1]] = simple_plan;
			plans.set(Object.assign($plans, obj));
		}
	...
	async function update_vote() {
		// collect all inputs
		let casts = query_selector_all("input.votes").filter((i) => {
		...
		let total = Object.values(new_votes).reduce(
			(total, v) => total + v,
			0.0
		);
		...
		let new_topic_data = await fetch(
